class Utils {
   ...
   */
  static getSignNum(number: number, positive: string, negative: string) {
    if (number > 0) {
      return positive;
    }
    if (number < 0) {
   ...
   */
  static deleteArrayRow(list: Array<any>, key: any, value: any) {
    let data = list;
    if (list) {
      data = list.filter((item) => item[key] !== value);
    }
   ...
   */
  static wildCardSearch(list: Array<any>, input: any) {
    const searchText = (item: Array<any>) => {
   ...
   */
  static getBreakPoint(screens: any): Array<any> {
    const breakpoints = [];
